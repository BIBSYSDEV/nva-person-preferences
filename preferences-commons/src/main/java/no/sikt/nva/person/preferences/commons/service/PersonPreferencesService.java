...
public class PersonPreferencesService {
  ...
  private static PersonPreferencesDao injectCreatedTimeStamp(
      PersonPreferencesDao personPreferencesDao) {
    return personPreferencesDao
        .copy()
  ...
  private static PersonPreferencesDao injectModifiedTimeStamp(
      PersonPreferences personPreferences, PersonPreferencesDao profile) {
  ...
  public PersonPreferences upsertPreferences(PersonPreferences personPreferences)
      throws NotFoundException {
    var primaryKey = primaryKey(personPreferences);
    var persistedDao =
        client.getItem(GetItemRequest.builder().tableName(tableName).key(primaryKey).build());
    return persistedDao.hasItem()
        ? updatePreferences(personPreferences)
        : createPreferences(personPreferences);
  }

  public PersonPreferences fetchPreferences(PersonPreferences personPreferences)
      throws NotFoundException {
  ...
  private PersonPreferences createPreferences(PersonPreferences personPreferences)
      throws NotFoundException {
    ...
    var request =
        TransactWriteItemsRequest.builder().transactItems(List.of(transactionItem)).build();
  ...
  private PersonPreferences updatePreferences(PersonPreferences personPreferences)
      throws NotFoundException {
  ...
  private Map<String, AttributeValue> primaryKey(PersonPreferences userPreferences) {
    return Map.of(
        PRIMARY_PARTITION_KEY, AttributeValue.fromS(userPreferences.personId().toString()));
  }

  private PersonPreferencesDao fetchPersonPreferences(PersonPreferences personPreferences)
      throws NotFoundException {
    var primaryKey = primaryKey(personPreferences);
    return new PersonPreferencesDao.Builder().fromDynamoFormat(getResourceByPrimaryKey(primaryKey));
  }

  private Map<String, AttributeValue> getResourceByPrimaryKey(
      Map<String, AttributeValue> primaryKey) throws NotFoundException {
    var result =
        client.getItem(GetItemRequest.builder().tableName(tableName).key(primaryKey).build());
    if (!result.hasItem()) {
