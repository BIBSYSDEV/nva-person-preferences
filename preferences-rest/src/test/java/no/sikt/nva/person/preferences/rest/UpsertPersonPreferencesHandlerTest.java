...
public class UpsertPersonPreferencesHandlerTest extends LocalPreferencesTestDatabase {
  ...
  void shouldCreatePersonPreferencesWhenDoesNotExist() throws IOException, NotFoundException {
    ...
    var updatePersonPreferences =
        existingPersonPreferences.copy().withPromotedPublications(List.of()).build();
    handler.handleRequest(createRequest(updatePersonPreferences), output, CONTEXT);

    assertThat(
        personPreferencesService.fetchPreferences(existingPersonPreferences).personId(),
  ...
  private InputStream createUnauthorizedRequest(PersonPreferences personPreferences)
  ...
  private InputStream createRequest(PersonPreferences personPreferences)
      throws JsonProcessingException {
    return new HandlerRequestBuilder<PersonPreferences>(dtoObjectMapper)
        .withUserName(randomString())
        .withCurrentCustomer(randomUri())
